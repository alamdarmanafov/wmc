      ...
      <div className="grid gap-5 lg:grid-cols-3">
              ...
              <ul className="space-y-2 text-sm">
                {communities.map((c) => (
                  <li key={c.id} className="flex items-center justify-between gap-2">
                    <Link href={`/communities?status=${c.status}`} className="font-medium text-gray-900 hover:text-brand">
                      {c.name}
                    </Link>
        ...
        <div className="space-y-5">
          <Card title="Reports about this user" count={reports.data?.length ?? 0}>
            ...
            <Link href={`/reports?target=${p.id}`} className="mt-3 inline-block text-xs font-medium text-brand hover:underline">
              Open in reports →
            </Link>
          </Card>
